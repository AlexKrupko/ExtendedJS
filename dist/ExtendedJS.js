...
!function () {
    ...
    function ClassListForNodeList(nodeList) {
        return {
            ...
            toggle: function toggle(className, condition) {
    ...
    var _ajaxDefaults = {
        ...
        beforeSend: function beforeSend() {}, // handler which will be called before request send
        success: function success() {}, // handler which will be called if request will execute successful
        error: function error() {}, // handler which will be called if request will end with error or get invalid response data
        complete: function complete() {}, // handler which will be called after end of request independent of request results
        progress: function progress() {}, // handler which will be called on each change of upload progress
        timeout: 0, // timeout limit for request
        timeoutError: function timeoutError() {}, // handler which will be called if request will end by timeout limit
        abort: function abort() {}, // handler which will be called if request will end by calling abort method
    ...
    window.ExtendedJS = window.ExJS = window.exjs = {
        ...
        cookies: {
             ...
             *  @param {String} [domain]
             */
            delete: function _delete(key, path, domain) {
         ...
         */
        scroll: function scroll(_scroll, duration, callback) {
            _scroll = Math.round(_scroll);
            duration = duration || 0;

            var steps = duration > 0 ? Math.ceil(duration / 1000 * 30) : 1,
                stepScroll = (_scroll - window.scrollY) / steps,
                currentStep = 0;

            function step() {
                window.scrollTo(0, ++currentStep >= steps ? _scroll : window.scrollY + stepScroll);
        ...
        ajax: function ajax(uri, options) {
            ...
            (typeof uri === 'undefined' ? 'undefined' : _typeof(uri)) === 'object' ? options = uri : options.uri = uri;

            // parses options and sets properties
            var P = {};
            for (var i in _ajaxDefaults) {
                P[i] = _typeof(options[i]) === _typeof(_ajaxDefaults[i]) ? options[i] : _ajaxDefaults[i];
            }
            ...
            if (P.contentType.toLowerCase() === 'json') {
                ...
                if (!(P.data instanceof FormData)) {
                    P.data = _typeof(P.data) === 'object' ? ajaxParseParams(P.data).join('&') : '';
            ...
            if (P.cache) {
                ...
                if (_typeof(ajaxCache[P.cache]) === 'object' && ajaxCache[P.cache].expires > new Date()) {
                    status = 'cache';
         ...
         */
        ajaxDefaults: function ajaxDefaults(properties) {
            if ((typeof properties === 'undefined' ? 'undefined' : _typeof(properties)) === 'object') {
                for (var i in properties) {
                    _typeof(properties[i]) === _typeof(_ajaxDefaults[i]) && (_ajaxDefaults[i] = properties[i]);
                }
            }
        }
    };
